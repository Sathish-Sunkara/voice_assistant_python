...
def wishMe():   # this handlz datetime.datetime.now().hour
    if 0 <= hour < 12:
    ...
    speak("I am your Assistant. loal 2.o0. how can i assist today ??")
...
def assistant():
    ...
    if search_bar.get() == "" :
        ...
        elif "joke" in searched_text:
            joke = pyjokes.get_joke()
            search_output.insert(0,joke)
            speak(joke)
            
        else:
            text = "searching is invalid"
            speak(str(text))
            search_output.insert(0,text)
...
search_bar.config(state=tk.DISABLED)


btn = tk.Button(text="start" ,fg="#36ff97" , bg="#11282e",command=assistant,font=('arial',25),height=1,width=10)
btn.pack(pady=10)
